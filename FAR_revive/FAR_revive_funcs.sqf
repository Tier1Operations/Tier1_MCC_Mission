...
FAR_HandleDamage_EH =
{
  private ["_unit", "_shooter", "_hitPart", "_damage", "_amountOfDamage", "_isUnconscious", "_lifeCount"];
  ...
  //};
	...
	_lastFrame = _unit getVariable ["OLM_hit_lastFrame", 0];

	if ( !( _lastFrame == diag_frameno ) && ( _hitPart == "") ) then 
	{
		// damage related to new event/bullet: reset start parameters
		_unit setVariable ["OLM_hit_frameStatus", [0,0,0,0]];
		_unit setVariable ["OLM_hit_lastFrame", diag_frameno];
		//_unit setVariable ["OLM_hit_lastDamage", 0 ];
		_unit setVariable ["OLM_hit_endLoop", false ];
		_unit setVariable ["OLM_hit_processLoop", false ];
		_newLoop = true;
	};

	_totalDamage = _unit getVariable ["OLM_hit_totalDamage", 0 ];
	//_lastDamage = _unit getVariable ["OLM_hit_lastDamage", 0 ];
	_frameStatus = _unit getVariable ["OLM_hit_frameStatus", [0,0,0,0]];
	_endLoop = _unit getVariable ["OLM_hit_endLoop", true ];
	
	//diag_log str [diag_frameno, _lastFrame, _this, _currentDamage, _lastDamage];
	
	/*
	diag_log str [
					(_hitPart in ["legs", "hands", "body", "head"]),
					(_newLoop), 
					(_endLoop), 
					(_frameStatus),
					(_damage > 0.1),
	...
	*/
	   ...
	   ) then 
	{
		switch ( _hitPart ) do
		{					
			case "head": // OLM_hit_headDamage
			{
				if (_damage > 0.15) then 
				{
					//player sideChat format ["HIT at HEAD - damage: %1", (round (_damage*100))/100];
					diag_log format ["%1 - HIT at HEAD - %2 - damage: %3", diag_frameno, name _unit, (round (_damage*100))/100];
					_unit setVariable ["OLM_hit_headDamage", (round (_damage*100))/100];
				}
				else
				{
					_unit setVariable ["OLM_hit_headDamage", 0];
				};
				
				_frameStatus set [0,1];
			};
			
			case "body": // OLM_hit_bodyDamage
			{
				if (_damage > 0.15) then 
				{
					//player sideChat format ["HIT at BODY - damage: %1", (round (_damage*100))/100];
					diag_log format ["%1 - HIT at BODY - %2 - damage: %3", diag_frameno, name _unit, (round (_damage*100))/100];
					_unit setVariable ["OLM_hit_bodyDamage", (round (_damage*100))/100];
				}
				else
				{
					_unit setVariable ["OLM_hit_bodyDamage", 0];
				};
				
				_frameStatus set [1,1];
			};
			
			case "hands": // OLM_hit_handDamage
			{
				if (_damage > 0.15) then 
				{
					//player sideChat format ["HIT at HANDS - damage: %1", (round (_damage*100))/100];
					diag_log format ["%1 - HIT at HANDS - %2 - damage: %3", diag_frameno, name _unit, (round (_damage*100))/100];
					_unit setVariable ["OLM_hit_handDamage", (round (_damage*100))/100];
				}
				else
				{
					_unit setVariable ["OLM_hit_bodyDamage", 0];
				...
				_frameStatus set [2,1];
			};
			
			case "legs":  // OLM_hit_legsDamage
			{
				if (_damage > 0.15) then 
				{
					//player sideChat format ["HIT at LEGS - damage: %1", (round (_damage*100))/100];
					diag_log format ["%1 - HIT at LEGS - %2 - damage: %3", diag_frameno, name _unit, (round (_damage*100))/100];
					_unit setVariable ["OLM_hit_legsDamage", (round (_damage*100))/100];
				}
				else
				{
					_unit setVariable ["OLM_hit_bodyDamage", 0];
				...
				_frameStatus set [3,1];
				
				// "legs" is last item in damage loop so now set endLoop and processLoop to true
				_unit setVariable ["OLM_hit_endLoop", true ];
				_unit setVariable ["OLM_hit_processLoop", true ];
			};
		};	

		_unit setDamage _totalDamage;
		_currentDamage = damage _unit;
		_unit setVariable ["OLM_hit_totalDamage", _currentDamage ];
	}
	else
	{
		//diag_log str ["no match", diag_frameno, name _unit, _hitPart, _currentDamage];
		_unit setDamage _currentDamage;
		
		switch ( _hitPart ) do
		{		
			case "head":  { _frameStatus set [0,1]; };
			case "body":  { _frameStatus set [1,1]; };
			case "hands": { _frameStatus set [2,1]; };
			case "legs":  { _frameStatus set [3,1]; };
		};		
	};
	
	_unit setVariable ["OLM_hit_frameStatus", _frameStatus];
	
	_endLoop = _unit getVariable ["OLM_hit_endLoop", false ];
	_processLoop = _unit getVariable ["OLM_hit_processLoop", false ];
	
	
	if ( _endloop && { _processLoop } ) then
	{
		_headDamage = _unit getVariable ["OLM_hit_headDamage", 0];
		_bodyDamage = _unit getVariable ["OLM_hit_bodyDamage", 0];
		_handDamage = _unit getVariable ["OLM_hit_handDamage", 0];
		_legsDamage = _unit getVariable ["OLM_hit_legsDamage", 0];
		
		_unit setVariable ["OLM_hit_processLoop", false ];
	
		if (alive _unit && {_currentDamage >= 1} && {_isUnconscious == 0} && {_lifeCount != CUP_Lives}) then 
		{
			//_unit setDamage 0;
			_unit allowDamage false;
			_unit setVariable ["CUP_lifeCount", _lifeCount + 1, true];
			[_unit, _shooter] spawn FAR_Player_Unconscious;
			
			_hitDamage = ((round ((random 1)*10))/10);
			
			diag_log format ["New damage: %1", _hitDamage];
		}
		else
		{
			_hitDamage = _currentDamage;

			[_headDamage, _bodyDamage, _handDamage, _legsDamage] spawn OLM_fn_SetImpact;

		};

diag_log str ["PROCESSING Damage: ", name _unit, _hitDamage, _currentDamage, _headDamage, _bodyDamage, _handDamage, _legsDamage];
...
OLM_fn_SetImpact =
{
	private["_headDamage", "_bodyDamage", "_handDamage", "_legsDamage"];
	
	_headDamage = _this select 0;
	_bodyDamage = _this select 1;
	_handDamage = _this select 2;
	_legsDamage = _this select 3;
	
	if ( _headDamage > 0 ) then
	{
		if ( _headDamage < OLM_hit_headDamageMaxImpact ) then 
		{
			_totalDamage = _unit getVariable ["OLM_hit_totalDamage", 0 ];
		//	_totalDamage = _totalDamage + OLM_hit_headDamageImpact;
		//	_unit setHit ["head", _headDamage];  // Unit will black out and blood stains appear on head
		//  _headDamage > 0.44 blood will appear; <0.45 blood will disappear
		//	_unit setVariable ["OLM_hit_headDamage", _headDamage];
		}
		else
		{
			_totalDamage = _unit getVariable ["OLM_hit_totalDamage", 0 ];
		};
		player globalChat "HEAD: nit will black out and blood stains appear on head";
	};

	if ( _bodyDamage > 0 ) then 
	{
		//if ( _bodyDamage < OLM_hit_bodyDamageMaxImpact ) then 
		//{
		//	_totalDamage = _totalDamage + OLM_hit_bodyDamageImpact;
		//	_unit setHit ["body", _bodyDamage];  // Unit will blur vision and blood stains appear on body
		//  _bodyDamage > 0.44 blood will appear; <0.45 blood will disappear
		//	_unit setVariable ["OLM_hit_bodyDamage", _bodyDamage];
		//};
		player globalChat "BODY: Unit will blur vision and blood stains appear on body";
	};
	
	if ( _handDamage > 0 ) then 
	{
		//if ( _handDamage < OLM_hit_handDamageMaxImpact ) then 
		//{
		//	_totalDamage = _totalDamage + OLM_hit_handDamageImpact;
		//	_unit setHit ["hands", _handDamage];  // Unit will start to shake aiming and blood stains appear on hands
		//  _handDamage > 0.34 blood will appear; <0.35 blood will disappear
		//	_unit setVariable ["OLM_hit_handDamage", _handDamage];
		//};
		player globalChat "HAND: Unit will start to shake aiming and blood stains appear on hands";
	};
	
	if ( _legsDamage > 0 ) then 
	{
		//if ( _legsDamage < OLM_hit_legsDamageMaxImpact ) then 
		//{
		//	_totalDamage = _totalDamage + OLM_hit_legsDamageImpact;
		//	_unit setHit ["legs", _legsDamage];  // Unit will not be able to run and blood stains appear on legs
		//  _legsDamage > 0.34 blood will appear; <0.35 blood will disappear
		//	_unit setVariable ["OLM_hit_legsDamage", _legsDamage];
		//};
		player globalChat "LEGS: Unit will not be able to run and blood stains appear on legs";
	};
};